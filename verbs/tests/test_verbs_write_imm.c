...
/* Post from stack-local descriptors, as llama.cpp does. The provider must
 * copy them before this function returns. */
static int post_stack_recv(struct ibv_qp *qp, void *addr, uint32_t length,
...
int main(int argc, char **argv)
{
     ...
     * ping-pongs). */
    struct ibv_send_wr swr, *sbad;
    if (post_stack_recv(qp, &xbuf_rx, sizeof(xbuf_rx), xmr_rx->lkey, 1)) {
        fprintf(stderr, "post_recv exch failed\n");
        return 1;
    }
    ...
    if (is_server) {
        /* The stack-local request is gone before the completion arrives. */
        if (post_stack_recv(qp, NULL, 0, 0, 3)) {
            fprintf(stderr, "post_recv imm failed\n");
            return 1;
        }
