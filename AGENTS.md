...
- Bench allocators: `odl_tb5_bench_dmabuf --allocator dmaheap|amdgpu|hip`. `amdgpu`/`hip` export a real GTT-backed amdgpu DMA-BUF via dlopen'd libdrm_amdgpu (no link deps); `hip` additionally imports it into HIP. Missing prerequisites → clean SKIP (exit 3), never a silent DMA-heap/memfd fallback. The bench uses **GTT placement**, not VRAM: amdgpu's exporter refuses `dma_buf_map_attachment` for the non-amdgpu NHI importer on VRAM BOs (driver logs `dma_buf_map_attachment(...) failed: -22`, first-transfer `-EINVAL` under `amdgpu`/`hip` → treated as a clean SKIP — a stack limitation, not an OdinLink bug; anything after the first transfer, or any other errno, stays a hard failure), and GTT DMA requires a **translated IOMMU** — rigs must boot `iommu=on` (pt/off leaves the NHI never completing RX DMA to GTT buffers). Gate runs an optional extra allocator round (`--extra-allocator`, default `hip`): SKIP on both hosts is recorded, not a failure.
