...
- Bench allocators: `odl_tb5_bench_dmabuf --allocator dmaheap|amdgpu|hip`. `amdgpu`/`hip` export real VRAM DMA-BUF via dlopen'd libdrm_amdgpu (no link deps); `hip` additionally imports it into HIP. Missing prerequisites → clean SKIP (exit 3), never a silent DMA-heap/memfd fallback. A first-transfer `-EINVAL` under `amdgpu`/`hip` (amdgpu exporter refuses to attach/map its DMA-BUF for the non-amdgpu NHI importer; driver logs `dma_buf_map_attachment(...) failed: -22`) is also treated as a clean SKIP — a stack limitation, not an OdinLink bug. Anything after the first transfer, or any other errno, stays a hard failure. Gate runs an optional extra allocator round (`--extra-allocator`, default `hip`): SKIP on both hosts is recorded, not a failure.
