...
- Bench allocators: `odl_tb5_bench_dmabuf --allocator dmaheap|amdgpu|hip`. `amdgpu`/`hip` export a real amdgpu DMA-BUF via dlopen'd libdrm_amdgpu (no link deps); `hip` additionally imports it into HIP. Missing prerequisites → clean SKIP (exit 3), never a silent DMA-heap/memfd fallback. GEM domain is picked by GPU kind (`--gpu-kind auto|igpu|dgpu`, detected from the VRAM heap size): an iGPU/APU allocates **GTT** (its carve-out VRAM can't be pinned to GTT for the NHI importer — first-transfer `-EINVAL`; GTT is the same physical memory on an APU), a dGPU allocates **VRAM** (the real GPU memory; amdgpu migrates the BO to GTT when the NHI maps it). If the exporter refuses VRAM on the first transfer the bench retries once with GTT before declaring SKIP (driver logs `dma_buf_map_attachment(...) failed: -22`; any other errno, or a post-first-transfer failure, stays a hard failure). GTT DMA requires a **translated IOMMU** — rigs must boot `iommu=on` (pt/off leaves the NHI never completing RX DMA to GTT buffers). Gate runs an optional extra allocator round (`--extra-allocator`, default `hip`): SKIP on both hosts is recorded, not a failure.
